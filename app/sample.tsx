// The sample dashboard, shared by /demo and by the home page when no wallet
// addresses are configured yet.
//
// Why the home page uses it: a fresh deployment with no addresses used to land
// on "nothing to show yet", which makes a correct install look broken. Someone
// who just deployed should see the thing working first and be told how to point
// it at their own wallets second. Nothing here touches the network or the
// database; the numbers are constants.
...
/** The banner a first-time visitor sees above the sample numbers. */
function Welcome() {
  return (
    <div className="card mb-6 border-neutral-700">
      <h2 className="mt-0">It works. These numbers are made up.</h2>
      <p className="muted mb-3">
        Your dashboard is deployed and you are signed in. It has no wallet addresses yet, so it is
        showing a sample portfolio to let you see what the real thing looks like.
      </p>
      <p className="mb-3 text-neutral-300">
        Pointing it at your own wallets takes about a minute and can be done from this phone.
      </p>
      <p className="mb-3"><a href="/setup" className="inline-block bg-neutral-100 text-neutral-900 font-medium px-3 py-2 rounded-md">Show me how</a></p>
      <p className="err text-xs mb-0">
        Public receiving addresses only. Never paste a private key or a seed phrase, here or anywhere
        else. This app has no field for one and no code that could use one.
      </p>
    </div>
  );
}

export default function Sample({ welcome = false }: { welcome?: boolean }) {
  return (
    <>
      {welcome && <Welcome />}
      <div className="flex items-baseline justify-between mb-4">
        <h1>Holdings <span className="badge badge-token ml-2 align-middle">sample data</span></h1>
